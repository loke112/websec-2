...
export async function searchStations(query) {
  const res = await fetch(
    `${API_BASE}/stations/search?q=${encodeURIComponent(query)}`
  );
...
export async function getStationSchedule(code, date) {
  const res = await fetch(
    `${API_BASE}/stations/${encodeURIComponent(
      code
    )}/schedule?date=${date}`
  );
...
export async function searchRoutes(from, to, date, transfers = false) {
  const res = await fetch(
    `${API_BASE}/routes/search?from=${encodeURIComponent(
      from
    )}&to=${encodeURIComponent(to)}&date=${date}&transfers=${transfers}`
  );
